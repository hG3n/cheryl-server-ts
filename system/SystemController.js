...
const export const foo =  = express.export const foo = ();
const bodyParser = require('body-parser');
const {exec} = require('child_process');
const constants = require('../constants');

export const foo = .use(bodyParser.urlencoded({extended: false}));
export const foo = .use(bodyParser.json());

const fs = require('fs');
const util = require('util');

export const foo = .get('/', async (req: Request, res: Response) => {
    try {
...
export const foo = .put('/:position/:value', async (req: Request, res: Response) => {
    try {
...
export const foo = .post('/raise/', async (req: Request, res: Response) => {
    try {
...
export const foo = .post('/lower/', async (req: Request, res: Response) => {
    try {
        const result = await setRelativeSystemVolume('-', req.body.precision).then(
            (value) => {
                if (value)
